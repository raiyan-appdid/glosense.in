...
const handleOnSuccess = (response: AuthCodeFlowSuccessResponse) => {
    loginWithEmail(response.access_token)
};
...
async function loginWithEmail(myToken: string) {
    await useFetch(`${apiUrl}/user/login-email`, {
        method: "POST",
        body: {
            google_token: myToken,
        },
        headers: {
...
<template>
    ...
    <!-- <button :disabled="!isReady" type="button" class="my-auto flex text-end border p-2 "> -->
        ...
        </span><span class="my-auto ml-3">Login with Google</span></button>
</template>
